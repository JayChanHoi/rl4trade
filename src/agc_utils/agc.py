    ...
    def step(self, closure=None):
        ...
        for group in self.optim.param_groups:
            for p in group['params']:
                ...
                param_norm = torch.max(unitwise_norm(p.detach()), torch.tensor(self.eps).to(p.device))
                grad_norm = unitwise_norm(p.grad.detach())
                max_norm = param_norm * self.clipping
                trigger = grad_norm > max_norm
                clipped_grad = p.grad * (max_norm / torch.max(grad_norm, torch.tensor(1e-8).to(grad_norm.device)))
